...
!(async () => {
    ...
    // 修改此处正则匹配，以适应新的网址 https://lx0.meituan.com/
    const queryStr = $request.url
       .match(/https:\/\/lx0.meituan.com\/queryTaskListInfoV.\?(.*)/)[1]
       .split('&')
        ...
        msg(e = t, s = "", i = "", r) {
            const o = t => {
                ...
                if ("object" == typeof t) {
                    ...
                    if (this.isQuanX()) {
                        let e = t["open-url"] || t.url || t.openUrl, s = t["media-url"] || t.mediaUrl, i = t["update-pasteboard"] || t.updatePasteboard;
